...
import Bracciocontrol #control braccio
import keyboard
...
def main():
    ...
    this_aruco_dictionary1 = cv2.aruco.getPredefinedDictionary(
        ARUCO_DICT[desired_aruco_dictionary1]
    )
    this_aruco_parameters1 = cv2.aruco.DetectorParameters()

    this_aruco_dictionary2 = cv2.aruco.getPredefinedDictionary(
        ARUCO_DICT[desired_aruco_dictionary2]
    )
    ...
    while(True):
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            Bracciocontrol.arm.write(b'P,90,45,180,180,90,73,20\n')
            break
        ...
        if keyboard.is_pressed('p'):
            ...
            # Get warped image size
            h, w, _ = img_wrapped.shape

            # ✅ Your real workspace size
            REAL_WIDTH = 750
            REAL_HEIGHT = 450

            # ✅ Convert pixel → mm
            x_mm = (cx / w) * REAL_WIDTH
            y_mm = (cy / h) * REAL_HEIGHT

            # ✅ Shift origin to center
            x_robot = x_mm - (REAL_WIDTH / 2)
            y_robot = y_mm

            print("Before compensation:", x_robot, y_robot)

            # ✅ Apply compensation
            x_final, y_final = Bracciocontrol.camera_compensation(x_robot, y_robot)

            print("After compensation:", x_final, y_final)

            # ✅ IMPORTANT: use compensated values
            cmd = "G:{x_final},{y_final},20,200,0,20\n"
            Bracciocontrol.arm.write(cmd.encode())
